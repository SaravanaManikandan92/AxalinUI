...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `Articles about ${category} | Axalin Blogs`,
    description: `Explore our informative articles about ${category}. Stay updated with valuable insights and tips.`,
    alternates: {
...
export default async function Category({ params, searchParams }: Props) {
  ...
  const res = await fetch(
    ...
    `https://axalingroup.com/api/blogs/category/${category}?page=${
      pageNumber - 1
    }&numberOfEntries=9`,
    {
      cache: "no-store",
    ...
    <div className="bg-white pb-24 sm:pb-32 py-16">
      <div className="mx-auto max-w-6xl px-6 lg:px-8">
        <div className="mx-auto max-w-2xl lg:max-w-none">
          <h1 className="text-2xl font-bold text-gray-900 sm:text-3xl">{`Posts by Category: ${category}`}</h1>
        </div>
